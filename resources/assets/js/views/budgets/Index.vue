<template>
  <div class="estimate-index-page budgets main-content">
    <div class="page-header">
      <h3 class="page-title">{{ $t('budgets.title') }}</h3>
      <ol class="breadcrumb">
      ...
      <div class="page-actions row">
        ...
        <router-link slot="item-title" class="col-xs-2" to="budgets/create">
          <base-button
            ...
            icon="plus"
            color="theme" >
            {{ $t('budgets.new_estimate') }}</base-button>
        </router-link>
      </div>
    </div>
      ...
      <div v-show="showFilters" class="filter-section">
        <div class="filter-container">
          ...
          <div class="filter-estimate">
            <label>{{ $t('budgets.estimate_number') }}</label>
            <base-input
    ...
    <div v-cloak v-show="showEmptyScreen" class="col-xs-1 no-data-info" align="center">
      ...
      <div class="row" align="center">
        <label class="col title">{{ $t('budgets.no_estimates') }}</label>
      </div>
      <div class="row">
        <label class="description col mt-1" align="center">{{ $t('budgets.list_of_estimates') }}</label>
      </div>
    ...
    <div v-show="!showEmptyScreen" class="table-container">
      <div class="table-actions mt-5">
        <p class="table-stats">{{ $t('general.showing') }}: <b>{{ budgets.length }}</b> {{ $t('general.of') }} <b>{{ totalEstimates }}</b></p>
  ...
  methods: {
    ...
    async fetchData ({ page, filter, sort }) {
      ...
      return {
        ...
        pagination: {
          totalPages: response.data.budgets.last_page,
          currentPage: page,
          count: response.data.budgets.count
        }
      }
    },
    async onMarkAsAccepted (id) {
      swal({
        ...
        text: this.$t('budgets.confirm_mark_as_accepted'),
        ...
        if (markedAsRejected) {
          ...
          if (response.data) {
            this.filters.status = ''
            this.$refs.table.refresh()
            window.toastr['success'](this.$tc('budgets.marked_as_accepted_message'))
          }
        }
      })
    },
    async onMarkAsRejected (id) {
      swal({
        title: this.$t('general.are_you_sure'),
        text: this.$t('budgets.confirm_mark_as_rejected'),
        ...
        if (markedAsRejected) {
          ...
          if (response.data) {
            this.filters.status = ''
            this.$refs.table.refresh()
            window.toastr['success'](this.$tc('budgets.marked_as_rejected_message'))
          }
        }
      })
    },
    ...
    async removeEstimate (id) {
      ...
      swal({
        title: this.$t('general.are_you_sure'),
        text: this.$tc('budgets.confirm_delete', 1),
        ...
        if (willDelete) {
          ...
          if (res.data.success) {
            ...
            this.filtersApplied = false
            this.resetSelectedEstimates()
            window.toastr['success'](this.$tc('budgets.deleted_message', 1))
    ...
    async convertInToinvoice (id) {
      swal({
        title: this.$t('general.are_you_sure'),
        text: this.$t('budgets.confirm_conversion'),
        ...
        if (willConvertInToinvoice) {
          ...
          if (res.data) {
            window.toastr['success'](this.$t('budgets.conversion_message'))
    ...
    async removeMultipleEstimates () {
      swal({
        title: this.$t('general.are_you_sure'),
        text: this.$tc('budgets.confirm_delete', 2),
        icon: '/assets/icon/trash-solid.svg',
        buttons: true,
        dangerMode: true
      }).then(async (willDelete) => {
        if (willDelete) {
          ...
          if (res.data.success) {
            ...
            this.resetSelectedEstimates()
            this.filtersApplied = false
            window.toastr['success'](this.$tc('budgets.deleted_message', 2))
          } else if (res.data.error) {
            window.toastr['error'](res.data.message)
          }
        }
    ...
    async onMarkAsSent (id) {
      swal({
        title: this.$t('general.are_you_sure'),
        text: this.$t('budgets.confirm_mark_as_sent'),
        icon: '/assets/icon/check-circle-solid.svg',
        buttons: true,
        dangerMode: true
        ...
        if (willMarkAsSent) {
          ...
          this.refreshTable()
          if (response.data) {
            window.toastr['success'](this.$tc('budgets.mark_as_sent_successfully'))
          }
        }
      })
    },
    async sendEstimate (id) {
      swal({
        title: this.$t('general.are_you_sure'),
        text: this.$t('budgets.confirm_send_estimate'),
        ...
        if (willSendEstimate) {
          ...
          if (response.data.success) {
            window.toastr['success'](this.$tc('budgets.send_estimate_successfully'))
            return true
          }
          if (response.data.error === 'user_email_does_not_exist') {
            window.toastr['error'](this.$tc('budgets.user_email_does_not_exist'))
            return true
          }
          window.toastr['error'](this.$tc('budgets.something_went_wrong'))
        }
      })
    }
  }
