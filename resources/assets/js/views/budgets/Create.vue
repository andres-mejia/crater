  ...
  <div class="estimate-create-page main-content">
    <form v-if="!initLoading" action @submit.prevent="submitEstimateData">
      <div class="page-header">
        <h3
          v-if="$route.name === 'budgets.edit'"
          class="page-title"
        >{{ $t('budgets.edit_estimate') }}</h3>
        <h3 v-else class="page-title">{{ $t('budgets.new_estimate') }}</h3>
        <ol class="breadcrumb">
          <li class="breadcrumb-item">
            <router-link slot="item-title" to="/admin/dashboard">{{ $t('general.home') }}</router-link>
          </li>
          <li class="breadcrumb-item">
            <router-link slot="item-title" to="/admin/budgets">{{ $tc('budgets.estimate', 2) }}</router-link>
          </li>
          <li
            v-if="$route.name === 'budgets.edit'"
            class="breadcrumb-item"
          >{{ $t('budgets.edit_estimate') }}</li>
          <li v-else class="breadcrumb-item">{{ $t('budgets.new_estimate') }}</li>
        </ol>
        <div class="page-actions row">
          <a
            v-if="$route.name === 'budgets.edit'"
            :href="`/budgets/pdf/${newEstimate.unique_hash}`"
            target="_blank"
          ...
          <base-button
            ...
            color="theme"
            type="submit"
          >{{ $t('budgets.save_estimate') }}</base-button>
        </div>
      </div>
          ...
          <div class="card">
            <div class="card-body">
              <div class="form-group">
                <label class="control-label">{{ $t('budgets.name') }}</label>
              ...
              <div class="form-group">
                <label>{{ $t('budgets.quantity') }}</label>
                <span class="text-danger">*</span>
              ...
              <div class="form-group">
                ...
                <base-select
                  ...
                  :searchable="true"
                  :show-labels="false"
                  :placeholder="$t('budgets.select_a_type')"
                  label="name"
                />
              </div>
              <div class="form-group">
                <label for="description">{{ $t('budgets.notes') }}</label>
                <base-text-area
              ...
              <div class="form-group">
                ...
                >{{ isEdit ? $t('budgets.update_Estimate') : $t('budgets.save_estimate') }}</base-button>
              </div>
            </div>
          </div>
        </div>
...
import BudgetStub from "../../stub/budget";
...
export default {
  ...
  computed: {
    ...
    isEdit() {
      if (this.$route.name === "budgets.edit") {
        return true;
      }
      return false;
    }
  ...
  methods: {
    ...
    submitSave(data) {
      this.addEstimate(data)
        .then(res => {
          if (res.data) {
            window.toastr["success"](this.$t("budgets.created_message"));
            this.$router.push("/admin/budgets");
          }
    ...
    submitUpdate(data) {
      this.updateEstimate(data)
        .then(res => {
          if (res.data) {
            window.toastr["success"](this.$t("budgets.updated_message"));
            this.$router.push("/admin/budgets");
          }
